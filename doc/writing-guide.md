...
### 2. **清晰的章節劃分 (模組化結構)**
   ...
   - **章節開場白原則**：為了改善段落間的承轉與流暢度，**每一個 `<h2>` 標籤後，都應補充一段完整豐富的引言知識 `<p>`**，用來提供背景知識、情境鋪陳，或引導讀者進入該章節的核心內容。
...
**實作方式：**
在目錄的 `<ul>` 列表中，為屬於子項目的 `<a>` 標籤加上 `sub-item` class 即可。
...
### 1. **提示框 (`.alert`)**
   ...
   - **排版說明**：提示框的 `<strong>` 標題為行內元素 (inline)，會與後續文字自然銜接在同一行。
